...
source_path: General_doc/gate1_SDD_VF_Onboarding_Copilot.pdf
document_type: pdf
role: general
...
Hệ thống VF-Onboarding Copilot là giải pháp Trợ lý AI chuyên biệt thiết kế dành riêng cho Đại lý Phân phối (ĐLPP) và Kỹ
thuật viên (KTV) trong hệ sinh thái xe điện VinFast. Hệ thống cung cấp khả năng tra cứu quy trình vận hành, mã lỗi kỹ thuật
(DTC), chính sách bán hàng - bảo hành, đồng thời hỗ trợ xử lý dữ liệu đa phương tiện (văn bản, bảng biểu, file ghi âm và
video hướng dẫn sửa chữa).
1.1. Các Chỉ Số Kỹ Thuật Bắt Buộc (SLA & Performance KPIs)
< 1.5s
E2E LATENCY P95
< 100ms
ROUTER LATENCY
...
> 98%
ROUTER ACCURACY
1.2. Sơ Đồ Kiến Trúc Luồng Dữ Liệu (High-Level Architecture)
 [User / KTV] ──► [Frontend Next.js / Role Selector (RBAC)]
 │
 ▼ (HTTP / WebSocket Stream)
 │ BACKEND FASTAPI ENGINE │
 │ [MODULE M0: MULTI-MODAL INGESTION & INPUT PROCESSING] │
 │ • Text/PDF/Docx • Excel Structurer • Video/Audio (Whisper+OCR) │
 │ [MODULE M2: 4-LAYER LIGHTWEIGHT ROUTER (<100ms)] │
 │ • Layer 1: Sanitizer ➔ L2: Trie Code Match ➔ L3: Embed ➔ L4: LLM │
 │ [MODULE M3: LANGGRAPH STATEGRAPH ORCHESTRATOR] │
 │ • Policy Copilot Node • Error Lookup Node • Static Form Node │
 │
 ▼ ▼
 │ CHROMA VECTOR DB │ │ SQLITE/POSTGRES │
 │ • docs & dtc │ │ • tickets & logs │
...
2.1. Ma Trận Xử Lý Đa Định Dạng (Processing Matrix)
2.2. Chi Tiết Luồng Xử Lý Video / Audio Pipeline
Quy trình đồng bộ Temporal Alignment:
Audio được bóc tách bằng Whisper tạo Transcript có timestamp $[t_{start}, t_{end}]$. Đồng thời, OpenCV cắt Keyframe màn
hình máy chẩn đoán để RapidOCR đọc mã lỗi. Các dữ liệu được hợp nhất theo mốc thời gian thực để tạo ra Chunk đa phương
tiện tích hợp.
// Ví dụ Cấu trúc JSON Output sau Module M0 xử lý Video:
{
  "chunk_id": "vid_vf8_dtc_001_c04",
  "source": "huong_dan_chua_loi_vf8.mp4",
  "timestamp_start": 124.5,
  "timestamp_end": 145.0,
  "transcript": "Kỹ thuật viên cắm thiết bị chẩn đoán vào cổng OBD-II...",
  "ocr_text": "DTC: P0A80 - Replace High Voltage Battery Pack",
  "metadata": {
    "car_model": "VF 8",
    "system": "BMS",
    "roles_allowed": ["technician", "lead_tech"]
  }
}
2.3. Quy Trình Chuẩn Hóa Văn Bản (Text Normalization Pipeline)
Unicode Normalization: Áp dụng chuẩn NFC cho toàn bộ tiếng Việt.
Định Dạng
Công Cụ Sử Dụng
...
Text (`.txt`, `.md`)
Native Python
Xóa khoảng trắng thừa, chuẩn hóa UTF-8
NFC, phân chia theo tiêu đề Header.
Markdown Chunks + Header
Context
Doc (`.docx`)
`python-docx`
Trích xuất Đoạn văn (Paragraph), chuyển
Bảng biểu (Table) thành dạng HTML
Table / Markdown.
Structured Text with HTML
Tables
PDF (`.pdf`)
`PyMuPDF` + `Docling`
Dual-path Parsing: Bóc tách luồng văn
bản gốc + khôi phục cấu trúc Bảng. Tự
động loại bỏ Header/Footer rác.
Clean Markdown + Page
Metadata
Spreadsheet (`.xlsx`,
`.csv`)
`pandas` + `openpyxl`
Chuyển từng Dòng thành đoạn văn có
ngữ cảnh schema: [Col_Name:
Value]. Tạo tóm tắt metadata.
Semi-structured JSON & Row
Chunks
Audio (`.mp3`, `.m4a`)
`Faster-Whisper` (v3)
Speech-to-Text kèm Timestamp chi tiết
(mốc 0.5s). Áp dụng VAD lọc nhiễu xưởng
sửa chữa.
